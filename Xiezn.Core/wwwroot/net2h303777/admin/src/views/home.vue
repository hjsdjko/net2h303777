...
<div class="content" :style='{"padding":"30px"}'>
	...
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('yiliaofeiyongjiesuan','首页总数')">
			...
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{yiliaofeiyongjiesuanCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>医疗费用结算总数</div>
			</div>
		</div>
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('yaopindingdan','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"rgba(44, 55, 66,1)","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-zhangjie18" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{yaopindingdanCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>药品订单总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type2" :style='{"alignContent":"flex-start","padding":"10px 20px","flexWrap":"wrap","background":"rebeccapurple","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="yiliaofeiyongjiesuanChart1" class="echarts1" v-if="isAuth('yiliaofeiyongjiesuan','首页统计')"></div>
		<div id="yaopindingdanChart1" class="echarts2" v-if="isAuth('yaopindingdan','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	data() {
		return {
            yiliaofeiyongjiesuanCount: 0,
            yaopindingdanCount: 0,
	...
	mounted(){
		this.init();
		this.getyiliaofeiyongjiesuanCount();
		this.yiliaofeiyongjiesuanChat1();
		this.getyaopindingdanCount();
		this.yaopindingdanChat1();
	},
	methods:{
		// 词云
		...
		getyiliaofeiyongjiesuanCount() {
			this.$http({
				url: `yiliaofeiyongjiesuan/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.yiliaofeiyongjiesuanCount = data.data
				}
			})
		},
		yiliaofeiyongjiesuanChat1() {
			this.$nextTick(()=>{

        var yiliaofeiyongjiesuanChart1 = echarts.init(document.getElementById("yiliaofeiyongjiesuanChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                yiliaofeiyongjiesuanChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yiliaofeiyongjiesuanChart1.resize();
                };
            }
        });
      })
		...
		getyaopindingdanCount() {
			this.$http({
				url: `yaopindingdan/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.yaopindingdanCount = data.data
				}
			})
		},
		yaopindingdanChat1() {
        ...
        var yaopindingdanChart1 = echarts.init(document.getElementById("yaopindingdanChart1"),'macarons');
        this.$http({
            url: `yaopindingdan/value/yaopinmingcheng/shuliang`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                yaopindingdanChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yaopindingdanChart1.resize();
                };
            }
        });
      })
