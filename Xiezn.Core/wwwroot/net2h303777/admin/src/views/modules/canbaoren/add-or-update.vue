<template>
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="参保人账号" prop="canbaorenzhanghao">
					<el-input v-model="ruleForm.canbaorenzhanghao" placeholder="参保人账号" clearable  :readonly="ro.canbaorenzhanghao"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="参保人账号" prop="canbaorenzhanghao">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="联系地址" prop="lianxidizhi">
					<el-input v-model="ruleForm.lianxidizhi" placeholder="联系地址" clearable  :readonly="ro.lianxidizhi"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="联系地址" prop="lianxidizhi">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				canbaorenzhanghao: [
					{ required: true, message: '参保人账号不能为空', trigger: 'blur' },
				],
				mima: [
					{ required: true, message: '密码不能为空', trigger: 'blur' },
				],
				canbaorenxingming: [
					{ required: true, message: '参保人姓名不能为空', trigger: 'blur' },
				],
				xingbie: [
				],
				nianling: [
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `canbaoren/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.canbaorenCrossAddOrUpdateFlag = false;
											this.parent.search();
