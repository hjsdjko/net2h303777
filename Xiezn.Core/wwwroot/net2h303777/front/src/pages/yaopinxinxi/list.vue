		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"55%","padding":"0 0px 0 0","flex":"1"}'>
				...
				<div class="title">{{item.yaopinleixing}}</div>
		      ...
		      <div class="desc" v-html="item.buliangfanying"></div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'药品类型',list:this.yaopinleixingOptions,tableName: 'yaopinleixing',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
