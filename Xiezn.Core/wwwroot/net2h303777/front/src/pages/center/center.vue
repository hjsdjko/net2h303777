...
<div class="center-preview" :style='{"padding":"0 7%","margin":"20px auto 40px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
	...
	<div class="info" :style='{"padding":"10px","boxShadow":"0 0px 0px rgba(0, 0, 0, 0.3)","margin":"20px 0 0 0","borderColor":"#1abc9e30","borderRadius":"0px","background":"none","borderWidth":"0 0 0 0px","width":"25%","borderStyle":"solid","height":"auto","order":"2"}'>
		...
		<div :style='{"borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 1px 0","width":"90%","fontSize":0,"borderStyle":"solid","height":"auto"}' v-if="userTableName=='canbaoren'">
		...
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='canbaoren'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
			<div :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}'>参保人账号</div>
			<div :style='{"fontSize":"14px","color":"#999","textAlign":"left","flex":1}'>{{sessionForm.canbaorenzhanghao}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='canbaoren'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
			<div :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}'>参保人姓名</div>
			<div :style='{"fontSize":"14px","color":"#999","textAlign":"left","flex":1}'>{{sessionForm.canbaorenxingming}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='canbaoren'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
    ...
    <el-tabs tab-position="left" :style='{"width":"72%","margin":"20px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","order":"1"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="80px">
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='canbaoren'" label="参保人账号" prop="canbaorenzhanghao">
            <el-input v-model="sessionForm.canbaorenzhanghao" placeholder="参保人账号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='canbaoren'" label="参保人姓名" prop="canbaorenxingming">
            <el-input v-model="sessionForm.canbaorenxingming" placeholder="参保人姓名" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='canbaoren'" label="性别" prop="xingbie">
            <el-select v-model="sessionForm.xingbie" placeholder="请选择性别" >
              <el-option v-for="(item, index) in dynamicProp.xingbie" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='canbaoren'" label="年龄" prop="nianling">
            <el-input v-model="sessionForm.nianling" placeholder="年龄" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='canbaoren'" label="身份证" prop="shenfenzheng">
            <el-input v-model="sessionForm.shenfenzheng" placeholder="身份证" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='canbaoren'" label="联系电话" prop="lianxidianhua">
            <el-input v-model="sessionForm.lianxidianhua" placeholder="联系电话" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='canbaoren'" label="联系地址" prop="lianxidizhi">
            <el-input v-model="sessionForm.lianxidizhi" placeholder="联系地址" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='canbaoren'" label="头像" prop="touxiang">
			<file-upload
			tip="点击上传头像"
			action="file/upload"
			:limit="1"
			...
			@change="canbaorentouxiangHandleAvatarSuccess"
			></file-upload>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
		...
		if ('canbaoren' == this.userTableName) {
			this.$set(this.sessionForm, 'canbaorenzhanghao', null);
		}
		if ('canbaoren' == this.userTableName) {
			this.$set(this.sessionForm, 'mima', null);
		}
		if ('canbaoren' == this.userTableName) {
			this.$set(this.sessionForm, 'canbaorenxingming', null);
		}
		if ('canbaoren' == this.userTableName) {
		...
		if ('canbaoren' == this.userTableName&&this.rules['canbaorenzhanghao']){
			this.rules['canbaorenzhanghao'].push({ required: true, message: '请输入参保人账号', trigger: 'blur' })
		}else if('canbaoren' == this.userTableName&&!this.rules['canbaorenzhanghao']) {
			this.$set(this.rules, 'canbaorenzhanghao', [{ required: true, message: '请输入参保人账号', trigger: 'blur' }]);
		}
		if ('canbaoren' == this.userTableName&&this.rules['mima']){
			this.rules['mima'].push({ required: true, message: '请输入密码', trigger: 'blur' })
		}else if('canbaoren' == this.userTableName&&!this.rules['mima']) {
			this.$set(this.rules, 'mima', [{ required: true, message: '请输入密码', trigger: 'blur' }]);
		}
		if ('canbaoren' == this.userTableName&&this.rules['canbaorenxingming']){
			this.rules['canbaorenxingming'].push({ required: true, message: '请输入参保人姓名', trigger: 'blur' })
		}else if('canbaoren' == this.userTableName&&!this.rules['canbaorenxingming']) {
			this.$set(this.rules, 'canbaorenxingming', [{ required: true, message: '请输入参保人姓名', trigger: 'blur' }]);
		}
		if ('canbaoren' == this.userTableName) {
    ...
    methods: {
      ...
      onSubmit(formName) {
		if(`canbaoren` == this.userTableName && this.sessionForm.touxiang!=null){
      ...
      canbaorentouxiangHandleAvatarSuccess(fileUrls) {
	  ...
	  async updatePassword(){
			this.$refs["passwordForm"].validate(async valid => {
				if (valid) {
					...
					if (this.userTableName == 'canbaoren') {
					}
