...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 20px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 1px","display":"flex","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"100px","padding":"0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>费用编号：</div>
        <el-input v-model="formSearch.feiyongbianhao" placeholder="费用编号" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"100px","padding":"0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>费用类型：</div>
        <el-input v-model="formSearch.feiyongleixing" placeholder="费用类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"55%","padding":"0 0px 0 0","flex":"1"}'>
				<div class="title">{{item.canbaorenxingming}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.feiyongbianhao != '') searchWhere.feiyongbianhao = '%' + this.formSearch.feiyongbianhao + '%';
        if (this.formSearch.feiyongleixing != '') searchWhere.feiyongleixing = '%' + this.formSearch.feiyongleixing + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`yiliaofeiyongjiesuan/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/yiliaofeiyongjiesuanDetail', query: params});
      },
